...
modelo <- lm(pnt_percent ~ perda_nao_tecnica, data = pnt_subestacoes_contribuicao)
...
eq_label <- sprintf("y = %.2f + %.2f·x", a, b)


# Gerar pontos da reta usando o modelo
reta_r2  <- data.frame(
  perda_nao_tecnica = seq(min(pnt_subestacoes_contribuicao$perda_nao_tecnica), max(pnt_subestacoes_contribuicao$perda_nao_tecnica), length.out = 100)
) %>%
  mutate(pnt_percent = a + b * perda_nao_tecnica)
...
hchart(
  ...
  "scatter",
  hcaes(y = pnt_percent, x = perda_nao_tecnica, name = subestacao_index, name2 = subestacoes, group = faixa)
) %>%
  # hc_colors(c("#72C8ED", "#FFAB4A", "#EA7542", "#005290")) %>%
  hc_colors(c("grey", "#FFCB8F", "#FFAB4A", "#EA7542", "#A20021")) %>%
  hc_chart(backgroundColor = "transparent") %>%
  hc_yAxis(
  ...
  hc_xAxis(
    # min = -0.5,
    # max = 4,
    title = list(text = "Perda não técnica (kWh)", style = list(fontFamily = "Montserrat")),
    labels = list(formatter = JS("function() { return Highcharts.numberFormat(this.value, 0, ',', '.'); }"))
  ) %>%
  hc_tooltip(formatter = JS("
    function() {
      return '<b>' + this.point.name2 + '</b><br>' +
             '<b>PNT (%) na subestação</b>: ' + Highcharts.numberFormat(this.y, 2, ',', '.') + ' %' + '<br>' +
             '<b>Perda não técnica (kWh)</b>: ' + Highcharts.numberFormat(this.x, 0, ',', '.') ;
    }
  ")) %>%
...
# 
  hc_add_series(data = reta_r2, type = "line",
                hcaes(x = perda_nao_tecnica, y = pnt_percent),
                name = "Regressão linear", color = "#000000", dashStyle = "ShortDash",
  ...
  labels = list(
    list(
      point = list(xAxis = 0, yAxis = 0, x = 270000, y = max(pnt_subestacoes_contribuicao$pnt_percent)+15),
      text = eq_label,
      backgroundColor = "rgba(255,255,255,0.7)",
      borderColor = "gray",
      style = list(fontSize = "12px", fontFamily = "Montserrat")
    )
  )
))
...
# cores_faixa <- c(
#   "Quadrante 1 (Baixo ASRO / Baixo PNT)" = "#f0d9e0",
#   "Quadrante 2 (Alto ASRO / Baixo PNT)"  = "#d699ad",
#   "Quadrante 3 (Baixo ASRO / Alto PNT)"  = "#b84c70",
#   "Quadrante 4 (Alto ASRO / Alto PNT)"   = "#7a0029"
# )
...
hchart(
  pnt_subestacoes_contribuicao,
  "scatter",
  hcaes(y = pnt_percent, x = perc_asro, name = subestacao_index, name2 = subestacoes, group = faixa)
) %>%
  hc_colors(c("#f0d9e0", "#d699ad", "#b84c70", "#7a0029")) %>%
  hc_chart(backgroundColor = "transparent") %>%
  hc_yAxis(
    title = list(text = "PNT (%) na subestação", style = list(fontFamily = "Montserrat")),
    plotLines = list(
  ...
  hc_xAxis(
    min = 0,
    max = 80,
    title = list(text = "Percentual de área coberta por ASRO", style = list(fontFamily = "Montserrat")),
    plotLines = list(
  ...
  hc_add_series(data = reta_r2, type = "line",
                ...
                name = "Regressão linear", color = "darkgrey", dashStyle = "ShortDash",
                marker = list(enabled = FALSE),
                enableMouseTracking = FALSE,
                showInLegend = TRUE) %>%
  hc_tooltip(formatter = JS("
  function() {
    return '<b>' + this.point.name2 + '</b><br>' +
           '<b>PNT (%) na subestação</b>: ' + Highcharts.numberFormat(this.y, 2, ',', '.') + ' %' + '<br>' +
           '<b>% coberto por ASRO</b>: ' + Highcharts.numberFormat(this.x, 2, ',', '.') + ' %';
  }
")) %>%
  hc_plotOptions(
    scatter = list(
  ...
  labels = list(
    list(
      point = list(xAxis = 0, yAxis = 0, x = 70, y = max(pnt_subestacoes_contribuicao$pnt_percent)+15),
      text = eq_label,
      backgroundColor = "rgba(255,255,255,0.7)",
      borderColor = "gray",
      style = list(fontSize = "12px", fontFamily = "Montserrat")
...
pnt_subestacoes_contribuicao_base %>%
  mutate(variacao_percentual = pnt_percent2023 - pnt_percent2022) %>%
  select(subestacoes, pnt_percent2022, pnt_percent2023, variacao_percentual, contribuicao_variacao_pnt_2223) %>%
  ...
  reactable(
    ...
    columns = list(
      ...
      pnt_percent2022 = colDef(name = "%PNT 2022", format = colFormat(digits = 2, percent = T)),
      pnt_percent2023 = colDef(name = "%PNT 2023", format = colFormat(digits = 2, percent = T)),
      variacao_percentual = colDef(name = "Variação percentual", format = colFormat(digits = 2, percent = T)),
      contribuicao_variacao_pnt_2223 = colDef(name = "Contribuição relativa", format = colFormat(digits = 2, percent = T))
    ),
    bordered = TRUE,
    striped = TRUE,
    highlight = TRUE,
    ...
    theme = reactableTheme(
      ...
      highlightColor = "#EAEAEA",
      cellPadding = "8px 12px",
      style = list(fontSize = "13px", fontFamily = "Montserrat, Arial, sans-serif"),
...
- <h5 style="color: black;"><strong>Alto percentual de PNT é generalizado na área de concessão</strong> <br>
<span style="font-weight: normal;">Quase todas as subestações da Light possuem percentual de PNT superior à média brasileira (5%).</span></h5>

- <h5 style="color: black;"><strong>Alto percentual de PNT em áreas com baixa presença de ASRO</strong> <br>
<span style="font-weight: normal;">A presença de ASRO, por si só, não é suficiente para explicar os altos percentuais de PNT nas subestações.</span></h5>

- <h5 style="color: black;"><strong>Elevação nos percentuais de PNT a partir de 2023</strong> <br> 
...
- <h5 style="color: black;"><strong>Subestações com alto percentual de PNT</strong> <br>
...
```{r, eval=FALSE, message=FALSE, warning=FALSE, fig.height=6.5, out.width='100%'}
...
```{r, eval=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=3.5, dpi=300}
